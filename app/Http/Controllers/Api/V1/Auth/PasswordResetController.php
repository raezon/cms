...
public function reset_password_submit(Request $request)
    {
       ...
       public function reset_change_password_request(Request $request)
    {
        $customer = User::where('phone', 'like', "%{$request['phone']}%")->select('id')->first();
        if (isset($customer)) {
            return response()->json(['status' => 'OK', 'message' => translate('Exist.')], 200);
        }
        return response()->json(['status' => 'Not found', 'message' =>'Customer not found!'], 404);
    }
    
    
     public function new_password_submit(Request $request)
    {
                $customer = User::where(['phone' => $request['phone']])->first();
                $customer->password = bcrypt($request['password']);
                $customer->save();
                return response()->json(['message' => translate('Password changed successfully.'),'status'=>"OK"], 200);
    }
